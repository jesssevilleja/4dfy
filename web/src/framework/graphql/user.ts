  ...
  const [mutate, { loading: isLoading, error }] = useSocialLoginMutation({
    onCompleted: (data) => {
      if (
        ...
        data?.socialLogin?.user?.roles?.length
      ) {
        setToken(data?.socialLogin?.accessToken);
        setAuthorized(true);
        client.resetStore();
      } else {
...
export function useOtpLogin() {
  ...
  const [otpLogin, { loading: isLoading }] = useOtpLoginMutation({
    onCompleted: (data) => {
      if (!data?.otpLogin?.accessToken && !data?.otpLogin?.user.roles?.length) {
        setServerError('text-otp-verify-failed');
        return;
      }
      setToken(data?.otpLogin?.accessToken!);
      setAuthorized(true);
      setOtpState({
        ...initialOtpState,
      });
...
export function useRegister() {
  ...
  const [registerUser, { loading: isLoading }] = useRegisterUserMutation({
    onCompleted: (data) => {
      if (data?.accessToken && data?.user?.roles?.length) {
  ...
  function register(values: RegisterUserInput) {
    registerUser({
      variables: {
        input: {
          ...values,
